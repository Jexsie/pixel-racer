...
const getMetadata = (): SignClientTypes.Metadata => ({
  name: "Pixel Racer",
  description: "Hedera Africa Hackathon 2025 game",
...
// Lazy initialization - only create DAppConnector when needed in browser
let dappConnector: DAppConnector | null = null;
let walletConnectInitPromise: Promise<void> | undefined = undefined;

const getDappConnector = (): DAppConnector => {
...
export const openWalletConnectModal = async () => {
  if (typeof window === "undefined") {
    console.warn("WalletConnect modal can only be opened in the browser");
    return;
  }

  await initializeWalletConnect();
  const connector = getDappConnector();
  await connector.openModal().then((x) => {
...
export const WalletConnectClient = () => {
  ...
  const syncWithWalletConnectContext = useCallback(() => {
    ...
    try {
      ...
      console.error("Error syncing WalletConnect:", error);
      setAccountId("");
      setIsConnected(false);
    }
